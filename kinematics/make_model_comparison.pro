PRO MAKE_MODEL_COMPARISON,REMCO=REMCO, JEANS=JEANS
...
modelfile='remco_models/outputs/bh6.80q0.73a0.00b0.00r0.55dc1.00f-10.00/ml4.90/nn_pq_kinem.out'
...
;first do velocities   
    ...
    CONTOUR, cont,xcoord,ycoord, levels=klevels,/fill,/xs,/ys,/nodata,charsize=2.0,xtitle='RA Offset ["]',ytitle='Dec Offset ["]',xminor=2,/iso,xmargin=[7,6],ymargin=[0.5,3.5],xcharsize=0.0001
...
;make the 255th color gray
    ...
    imgunder,velcolimage
    xyouts,[-0.95],[0.9],['Data'],chars=2,color=235

    axis,yaxis=0,ychars=0.00000001,/ysty
    axis,xaxis=0,xchars=0.00000001,/xsty
    axis,/xaxis,xrange=[xmin,xmax],/xsty,xtitle='RA Offset [pc]',charsize=2.0

    contour,cont,xcoord,ycoord,/overplot,levels=klevels,thick=4
    COLORBAR,range=[min(vellevels),max(vellevels)+dellev],format='(I4)',/vertical,right=1,divisions=5,charsize=2.0,title='Relative Velocity [km/sec]',POSITION=[!X.WINDOW[1],!Y.WINDOW[0],!X.WINDOW[1]+0.03,!Y.WINDOW[1]]
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;now do dispersions
    ...
    CONTOUR, cont,xcoord,ycoord, levels=levels,/fill,/xs,/ys,/nodata,charsize=2.0,xtitle='RA Offset ["]',ytitle='Dec Offset ["]',xminor=2,/iso,xmargin=[7,6],ymargin=[0.5,3.5],xcharsize=0.0001
;make the 255th color gray
    LOADCT,33,/silent
    R_CURR[255]=150 & G_CURR[255]=150 & B_CURR[255]=150
    R_CURR[0]=0 & G_CURR[0]=0 & B_CURR[0]=0
...
;levels at 1-mag intervals
    contour,cont,xcoord,ycoord,/overplot,levels=klevels,thick=4
    COLORBAR,range=[min(vellevels),max(vellevels)+dellev],format='(I4)',/vertical,right=1,divisions=5,charsize=2.0,title='Dispersion [km/sec]',POSITION=[!X.WINDOW[1],!Y.WINDOW[0],!X.WINDOW[1]+0.03,!Y.WINDOW[1]]
...
;first do velocities   
    ...
    dellev=(maxlev-minlev)/nlev
    vellevels=findgen(nlev)*dellev+minlev
    velcontimage=mvelimage
    low=WHERE(velimage LE minlev,nlow)
    IF (nlow GT 0) THEN velcontimage[low]=!VALUES.F_NAN
    high=WHERE(velimage GT maxlev,nhigh)
    IF (nhigh GT 0) THEN velcontimage[high]=!VALUES.F_NAN
    CONTOUR, cont,xcoord,ycoord, levels=klevels,/fill,/xs,/ys,/nodata,charsize=2.0,xtitle='RA Offset ["]',ytitle='Dec Offset ["]',xminor=2,/iso,xmargin=[7,6],ymargin=[3.5,0.5]
    pixelscale=SIN(1.0D0/3600./180.*!DPI)*10.^((dms[i]+5.)/5.)
    print,galaxies[i],pixelscale
    xmin=MIN(xcoord)*pixelscale
    xmax=MAX(xcoord)*pixelscale
...
;make the 255th color gray
    ...
    IF (nreplace GT 0) THEN velcolimage[replace]=255
    imgunder,velcolimage
    xyouts,[-0.95],[0.9],['Model'],chars=2,color=235
    axis,yaxis=0,ychars=0.00000001,/ysty
    axis,xaxis=0,xchars=0.00000001,/xsty
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    dellev=(maxlev-minlev)/nlev
    vellevels=findgen(nlev)*dellev+minlev
    dispcontimage=mdispimage
    ind=WHERE(dispimage GT maxlev,nhigh)
    IF (nhigh GT 0) THEN dispcontimage[ind]=maxlev-0.2
    ind=WHERE(dispimage LT minlev,nlow)
    IF (nlow GT 0) THEN dispcontimage[ind]=!VALUES.F_NAN

    CONTOUR, cont,xcoord,ycoord, levels=levels,/fill,/xs,/ys,/nodata,charsize=2.0,xtitle='RA Offset ["]',ytitle='Dec Offset ["]',xminor=2,/iso,xmargin=[7,6],ymargin=[3.5,0.5]
;make the 255th color gray
    LOADCT,33,/silent
    R_CURR[255]=150 & G_CURR[255]=150 & B_CURR[255]=150
    R_CURR[0]=0 & G_CURR[0]=0 & B_CURR[0]=0
