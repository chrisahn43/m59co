PRO INT_SPECTRA_CHECK_ROT,minradius,maxradius,xcen,ycen
  ...
  outfile='m59co_goodmay_int_fc'+STRTRIM(FIX(minradius),2)+'-'+STRTRIM(FIX(maxradius),2)+'.fits'
  ...
  shift=[0,0.75,1.5,2.25,3.25,4.5,6.125,11];,21]
...
;  stop
  ...
  for i=0,cubesize[2]-1 do begin
     ...
     aper,(cube[*,*,i]-skyval),xcen-max(shift),ycen,maxflux,fluxerr,sky,skyerr,1.0,[maxradius],[20,25],[-32000,32000],/silent,setskyval=0.,/flux,/exact,/NAN
     if (minradius gt 0.5) then begin
        aper,(cube[*,*,i]-skyval),xcen-shift[n_elements(shift)-2],ycen,minflux,fluxerr,sky,skyerr,1.0,[minradius],[20,25],[-32000,32000],/silent,setskyval=0.,/flux,/exact,/NAN
        flux=maxflux-minflux
     endif else flux=maxflux
     aper,(cube[*,*,i]),xcen-max(shift),ycen,maxrawflux,rawfluxerr,rawsky,rawskyerr,1.0,[maxradius],[20,25],[-32000,32000],/silent,setskyval=0.,/flux,/exact,/NAN
     if (minradius gt 0.5) then begin
        aper,(cube[*,*,i]),xcen-shift[n_elements(shift)-2],ycen,minrawflux,rawfluxerr,rawsky,rawskyerr,1.0,[minradius],[20,25],[-32000,32000],/silent,setskyval=0.,/flux,/exact,/NAN
        rawflux=maxrawflux-minrawflux
     endif else rawflux=maxrawflux

     aper,(varcube[*,*,i]),xcen-max(shift),ycen,varflux,varfluxerr,varsky,varskyerr,1.0,[maxradius],[20,25],[-32000,32000],/silent,setskyval=0.,/flux,/exact,/nan
     outspec[i]=flux
     outvar[i]=varflux
